...
function reverseInteger(num) {
  ...
  const s = String(Math.abs(num)).split('').reverse().join('');
...
function isCreditCardNumber(ccn) {
  const digits = String(ccn).replace(/\D/g, '').split('').reverse().map(Number);
...
function getCommonDirectoryPath(pathes) {
  ...
  const split = pathes.map((p) => p.split('/'));
  const minLen = Math.min(...split.map((s) => s.length));
  const common = [];
  for (let i = 0; i < minLen; i += 1) {
    const seg = split[0][i];
    if (split.every((arr) => arr[i] === seg)) {
      common.push(seg);
    } else {
      break;
    }
  ...
  // If only leading '' (root) is common
...
function getMatrixProduct(m1, m2) {
  ...
  return Array.from({ length: rows }, (__row, i) =>
    Array.from({ length: cols }, (__col, j) => {
      let sum = 0;
