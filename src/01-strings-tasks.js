...
function removeFirstOccurrences(str, value) {
  return String(str).replace(value, '');
}
...
function getRectangleString(width, height) {
  ...
  if (w < 2 || h < 2) {
    // minimal size still produces borders
    const top = '┌┐\n';
    const bottom = '└┘\n';
    return top + bottom.repeat(Math.max(0, h - 1));
  }
  const top = `┌${'─'.repeat(w - 2)}┐\n`;
  const middle = `│${' '.repeat(w - 2)}│\n`;
  const bottom = `└${'─'.repeat(w - 2)}┘\n`;
  return top + middle.repeat(h - 2) + bottom;
}
